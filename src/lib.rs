...
use winnow::binary::{be_u8, be_u16, be_u24, be_u32, length_and_then, length_repeat, length_take};
...
pub struct PresentationComposition {
    ...
    pub palette_id: u8,
    pub commposition_objects: Vec<CompositionObject>,
}
...
#[derive(PartialEq, Eq)]
pub struct RunLengthEncodedData(Vec<u8>);

impl std::fmt::Debug for RunLengthEncodedData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("RunLengthEncodedData").finish()
    }
}
...
fn parse_segment(input: &mut &[u8]) -> PResult<Segment> {
    ...
    let (pts, dts, contents) = (
        ...
        .parse_next(input)?;
    println!("Parsed segment: {:?}", contents);
...
fn parse_object_definition_segment(input: &mut &[u8]) -> PResult<ObjectDefinition> {
    let (id, version, last_in_sequence, (width, height, data)) = length_and_then(
        ...
        (
            ...
            length_and_then(be_u24, (be_u16, be_u16, repeat(0.., be_u8))),
        ),
    )
    .parse_next(input)?;
    Ok(ObjectDefinition {
...
fn parse_palette_definition_segment(input: &mut &[u8]) -> PResult<PaletteDefinition> {
    Ok(PaletteDefinition::from_tuple(
        length_and_then(be_u16, (be_u8, be_u8, repeat(0.., parse_palette_entry)))
            .parse_next(input)?,
    ))
}

fn parse_palette_entry(input: &mut &[u8]) -> PResult<PalletteEntry> {
    Ok(PalletteEntry::from_tuple(
